...
(*Code for registers largely modified from example registers.ml*)
let aget = Array.get
let aset = Array.set
let amake = Array.make
(*Creates a symbol table array of 256 empty registers for implementation*) 
let symbol_table = amake 256 (false, Bigint.zero) 

let st sym value = aset symbol_table sym (true, value)
       
let ld sym thestack =
    let entry = aget symbol_table sym
     in match entry with
