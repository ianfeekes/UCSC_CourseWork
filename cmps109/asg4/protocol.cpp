...
/*For the client and the server to communicate, a protocol needs to 
 *be established. Each message must be framed in terms of a header
 *and a payload. The header always consists of a struct of size 64
 *bytes. All messages between the client and the server consist of
 *these 64 bytes, possibly followed by a payload. For alignment 
 *purposes, the nbytes field is first.*/ 
